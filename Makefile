...
osx: version $(RELEASE_DIR)
	cd $(GAME_DIR) && godot --export-debug "osx" "../$(RELEASE_DIR)/$(OSX_TARGET)$(EXT_OSX)"

linux: version $(RELEASE_DIR)
	cd $(GAME_DIR) && godot --export-debug "ubuntu64" "../$(RELEASE_DIR)/$(LINUX_TARGET)$(EXT_GNU)"
...
windows: version $(RELEASE_DIR)
	cd $(GAME_DIR) && godot --export-debug "windows" "../$(RELEASE_DIR)/$(WINDOWS_TARGET)$(EXT_WIN)"
